...
class MultiColumnWizard extends Widget
{
    ...
    public function generate()
    {
        ...
        foreach ($this->columnFields as $strKey => $arrField)
        {
            ...
            {
                if ($arrField['label'][0]) $arrHeaderItems[] = $arrField['label'][0];
                else $arrHeaderItems[] = $label;
            }
        }
        ...
        for ($i = 0; $i < $intNumberOfRows; $i++)
        {
            ...
            foreach ($this->columnFields as $strKey => $arrField)
            {
                if (!$arrField['eval']['columnPos']) unset($arrField['label']);
                ...
                $objWidget = new $strClass($this->prepareForWidget($arrField, $arrField['name'], $arrField['value']));
                if (isset($arrField['eval']['tl_class'])) $objWidget->class = $arrField['eval']['tl_class'];
                ...
                else
                {
                    ...
                    + 'class' => (isset($arrField['eval']['tl_class'])) ? $arrField['eval']['tl_class'] : false
                }
            ...
            {
                $arrReturnItems[$itemKey] = '<td' . (($itemValue['valign'] != '') ? ' valign="' . $itemValue['valign'] . '" ' : '') . (($itemValue['class']) ? ' class="' . $itemValue['class'] . '"' : '') . '>' . $itemValue['entry'] . '</td>';
            }

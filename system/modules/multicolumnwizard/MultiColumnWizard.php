...
class MultiColumnWizard extends Widget implements uploadable
{
    ...
    protected function validator($varInput)
    {
        ...
        for ($i = 0; $i < count($varInput); $i++) {
            ...
            foreach ($this->columnFields as $strKey => $arrField) {
                ...
                if (!$objWidget->hasErrors() && ($rgxp == 'date' || $rgxp == 'time' || $rgxp == 'datim')
                    && $varValue != '') {
    ...
    public function generate()
    {
        ...
        // Change the order
        if ($this->Input->get($this->strCommand) && is_numeric($this->Input->get('cid'))
            && $this->Input->get('id') == $this->currentRecord) {
            ...
            if ($GLOBALS['TL_DCA'][$this->strTable]['config']['dataContainer'] == 'File') {
                ...
                if ($GLOBALS['TL_DCA'][$this->strTable]['config']['dataContainer'] == 'Table') {
                    if (is_array($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['save_callback'])) {
                        ...
                        foreach (
                            $GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['save_callback'] AS
                            $callback
                        ) {
                            $this->import($callback[0]);
            ...
            foreach ($this->columnFields as $strKey => $arrField) {
                ...
                if ($objWidget === null) {
                    ...
                    if ($arrField['eval']['datepicker']) {
                        ...
                        $datepicker =
                            ' <img src="system/modules/multicolumnwizard/html/img/datepicker.gif" width="20" height="20" alt="" id="toggle_'
                            . $objWidget->id . '" style="vertical-align:-6px;">
                          <script>
                              window.datepicker_' . $this->strName . '_' . $strKey . ' = new DatePicker(\'#ctrl_'
                            . $objWidget->id . '\', {
                              allowEmpty:true,
    ...
    protected function getMcWDatePickerString($strId, $strKey, $rgxp)
    {
        if (version_compare(VERSION, '2.11', '<')) {
            ...
            return ' <img src="system/modules/multicolumnwizard/html/img/datepicker.gif" width="20" height="20" alt="" id="toggle_'
                   . $strId . '" style="vertical-align:-6px;">
                          <script>
                        window.addEvent("domready", function() {
            ...
            return ' <img src="system/modules/multicolumnwizard/html/img/datepicker.gif" width="20" height="20" alt="" id="toggle_'
                   . $strId . '" style="vertical-align:-6px;cursor:pointer;">
                        <script>
                        window.addEvent("domready", function() {
    ...
    protected function initializeWidget(&$arrField, $intRow, $strKey, $varValue)
    {
        ...
        if ($arrField['inputType'] == 'textarea' && $arrField['eval']['rte'] == '') {
            $xlabel .= ' ' . $this->generateImage(
                    ...
                    'title="' . specialchars($GLOBALS['TL_LANG']['MSC']['wordWrap'])
                    . '" class="toggleWrap" onclick="Backend.toggleWrap(\'ctrl_' . $this->strId . '_row' . $intRow . '_'
                    . $strKey . '\');"'
                );
        }
        ...
        if ($arrField['eval']['helpwizard']) {
            if (version_compare(VERSION, '3.1', '<')) {
                $xlabel .= ' <a href="contao/help.php?table=' . $this->strTable . '&amp;field=' . $this->strField
                           . '" title="' . specialchars($GLOBALS['TL_LANG']['MSC']['helpWizard'])
                           . '" data-lightbox="help 610 80%">' . $this->generateImage(
                        'about.gif',
                ...
                $xlabel .= ' <a href="contao/help.php?table=' . $this->strTable . '&amp;field=' . $this->strField
                           ...
                           . specialchars(str_replace("'", "\\'", $arrField['label'][0]))
        ...
        if ($arrField['inputType'] == 'fileTree' || $arrField['inputType'] == 'pageTree') {
            ...
            $xlabel .= ' <a href="' . $strContaoPrefix . 'files.php' . $path . '" title="' . specialchars(
                    $GLOBALS['TL_LANG']['MSC']['fileManager']
                ) . '" data-lightbox="files 765 80%">' . $this->generateImage(
                    'filemanager.gif',
                    $GLOBALS['TL_LANG']['MSC']['fileManager'],
                    'style="vertical-align:text-bottom;"'
                ) . '</a>';
        ...
        elseif ($arrField['inputType'] == 'tableWizard') {
            $xlabel .= ' <a href="' . $this->addToUrl('key=table') . '" title="' . specialchars(
                    $GLOBALS['TL_LANG']['MSC']['tw_import'][1]
                ) . '" onclick="Backend.getScrollOffset();">' . $this->generateImage(
                    'tablewizard.gif',
            ...
            $xlabel .= ' ' . $this->generateImage(
                    'demagnify.gif',
                    '',
                    'title="' . specialchars($GLOBALS['TL_LANG']['MSC']['tw_shrink'])
                    . '" style="vertical-align:text-bottom; cursor:pointer;" onclick="Backend.tableWizardResize(0.9);"'
                ) . $this->generateImage(
                    'magnify.gif',
                    '',
                    'title="' . specialchars($GLOBALS['TL_LANG']['MSC']['tw_expand'])
        ...
        elseif ($arrField['inputType'] == 'listWizard') {
            $xlabel .= ' <a href="' . $this->addToUrl('key=list') . '" title="' . specialchars(
                    $GLOBALS['TL_LANG']['MSC']['lw_import'][1]
                ) . '" onclick="Backend.getScrollOffset();">' . $this->generateImage(
                    'tablewizard.gif',
                    $GLOBALS['TL_LANG']['MSC']['tw_import'][0],
                    'style="vertical-align:text-bottom;"'
        ...
        if (is_array($arrField['input_field_callback'])) {
            ...
            return $this->{$arrField['input_field_callback'][0]}->{$arrField['input_field_callback'][1]}(
                $this,
                $xlabel
            );
        }
        ...
        if (isset($arrField['eval']['rowClasses'][$intRow])) {
            $arrField['eval']['tl_class'] =
                trim($arrField['eval']['tl_class'] . ' ' . $arrField['eval']['rowClasses'][$intRow]);
        }
        ...
        if (version_compare(VERSION, '3.1', '<')) {
            ...
            $objWidget = new $strClass(
                \MultiColumnWizard::getAttributesFromDca(
                    $arrData,
                    $arrField['name'],
                    $arrField['value'],
                    $arrField['strField'],
    ...
    public function handleDcGeneral($arrData, $strName)
    {
        ...
        if (is_subclass_of($this->objDca, 'ContaoCommunityAlliance\DcGeneral\EnvironmentAwareInterface')) {
            ...
            if (method_exists($environment, 'getEventPropagator')) {
                $event =
                    new \ContaoCommunityAlliance\DcGeneral\Contao\View\Contao2BackendView\Event\GetPropertyOptionsEvent(
                        $environment, $this->objDca->getModel()
                    );
    ...
    protected function generateTable(
        ...
        foreach ($this->columnFields as $strKey => $arrField) {
            ...
            if ($arrField['eval']['columnPos']) {
                ...
                $strHeaderItem .= (is_array($arrField['label'])) ? $arrField['label'][0]
                    : ($arrField['label'] != null ? $arrField['label'] : $strKey);
                $strHeaderItem .= ((is_array($arrField['label']) && $arrField['label'][1] != '') ? '<span title="'
                                                                                                   . $arrField['label'][1]
                                                                                                   . '"><sup>(?)</sup></span>' : '');
        ...
        $return = '
<table' . (($this->style) ? (' style="' . $this->style . '"') : ('')) . ' data-operations="maxCount['
                  . ($this->maxCount ? $this->maxCount : '0') . '] minCount['
                  . ($this->minCount ? $this->minCount : '0') . '] unique[' . implode(',', $arrUnique) . '] datepicker['
                  . implode(',', $arrDatepicker) . '] colorpicker[' . implode(',', $arrColorpicker) . ']" id="ctrl_'
                  . $this->strId . '" class="tl_modulewizard multicolumnwizard">';
  ...
  <tbody>';
        ...
        foreach ($arrItems as $k => $arrValue) {
            ...
            foreach ($arrValue as $itemKey => $itemValue) {
                ...
                $return .= '<td' . ($itemValue['valign'] != '' ? ' valign="' . $itemValue['valign'] . '"' : '')
                           . ($itemValue['tl_class'] != '' ? ' class="' . $itemValue['tl_class'] . '"' : '') . '>'
                           . $itemValue['entry'] . '</td>';
            }

            // insert buttons at the very end
            $return .= '<td class="operations col_last"' . (($this->buttonPos != '') ? ' valign="' . $this->buttonPos
                                                                                       . '" ' : '') . '>' . $strHidden;
    ...
    protected function generateDiv(
        ...
        foreach ($this->columnFields as $strKey => $arrField) {
            ...
            $arrHeaderItems[] =
                sprintf('<div class="%s">%s</div>', $strKey, ($arrField['label'][0] ? $arrField['label'][0] : $strKey));
        }

        $return = '<div' . (($this->style) ? (' style="' . $this->style . '"') : '') . ' data-operations="maxCount['
                  . ($this->maxCount ? $this->maxCount : '0') . '] minCount['
                  . ($this->minCount ? $this->minCount : '0') . '] unique[' . implode(',', $arrUnique) . '] datepicker['
                  . implode(',', $arrDatepicker) . '] colorpicker[' . implode(',', $arrColorpicker) . ']" id="ctrl_'
                  . $this->strId . '" class="tl_modulewizard multicolumnwizard">';
        ...
        foreach ($arrItems as $itemKey => $itemValue) {
            ...
            $arrReturnItems[$itemKey] =
                '<div' . ($itemValue['tl_class'] != '' ? ' class="' . $itemValue['tl_class'] . '"' : '') . '>'
                . $itemValue['entry'] . '</div>';
        }
    ...
    protected function generateButtonString($level = 0)
    {
        ...
        foreach ($this->arrButtons as $button => $image) {
            ...
            $return .= '<a data-operations="' . $button . '" href="' . $this->addToUrl(
                    '&' . $this->strCommand . '=' . $button . '&cid=' . $level . '&id=' . $this->currentRecord
                ) . '" class="widgetImage" title="' . $GLOBALS['TL_LANG']['MSC']['tw_r' . specialchars($button) . '']
                       . '">' . $this->generateImage(
                    $image,
